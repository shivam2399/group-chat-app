...
const sendPersonalMessage = async (req, res) => {
  try {
    ...
    return res.status(error.statusCode || 500).json({
      success: false,
      message: error.message || "Failed to send personal message",
    });
  }
};
...
const getPersonalMessages = async (req, res) => {
  try {
    ...
    const messages = await personalMessageService.getPersonalMessages(
      userId,
      Number(otherUserId),
      page,
      limit,
    ...
    return res.status(error.statusCode || 500).json({
      success: false,
      message: error.message || "Failed to fetch personal messages",
    });
  }
};
...
const createPersonalMediaMessage = async (req, res) => {
  try {
    ...
    return res.status(201).json({
      success: true,
      message: "Personal media message created successfully",
      data: message,
    });
  } catch (error) {
    console.error("CREATE PERSONAL MEDIA MESSAGE ERROR:", error);
    return res.status(error.statusCode || 500).json({
      success: false,
      message: error.message || "Failed to create personal media message",
    });
  }
};
